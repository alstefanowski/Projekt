...
char *msg = "Wylogowanie z systemu";
User users_table[MAX_USERS];
int server_fd;

pid_t clientid;
User user;
// Zapisywanie informacji do logu, z biezaca data oraz godzina
int sygnal;
...
void receive_mess(int fifo)
{
    ...
    {
        if(strcmp(message.odbiorca, "Serwer") == 0) {
            if(strcmp(message.message, "shutdown") == 0) {
        ...
        else
        {
            ...
            if (odbiorca_fd != -1)
                write(odbiorca_fd, (char*)&message, sizeof(Message));
            else
...
int main(int argc, char **argv)
{
    ...
    if (!strcmp(argv[1], "--start"))
    {
        if (fork() == 0)
        {
            ...
            {
                Message message;
                if (read(server_fd, (char *)&message, sizeof(Message)) > 0)
                {
                    for (int i = 0; i < num_registered_users; i++)
                    {
                        if (!strcmp(users_table[i].nickname, message.odbiorca))
                        {
                            write(users_table[i].fifo_fd, (char *)&message, sizeof(Message));
                            break;
                        }
                    }
                }
    ...
    else if (!strcmp(argv[1], "--login"))
    {
        ...
        {
            send_to_log("zbyt duzo uzytkownikow");
            exit(1);
        }
        int index = -1;
        for (int i = 0; i < num_registered_users; i++)
        ...
        if (index == -1)
        {
            ...
            int id = atoi(argv[2]);
            sprintf(fifo_name, "/home/kali/Projekt/Fifo%d", id);
        ...
        else
        {
            ...
            strcpy(message.odbiorca, "Server");
            while (1)
            {
